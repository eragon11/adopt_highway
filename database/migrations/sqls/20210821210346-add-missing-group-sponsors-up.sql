CREATE
OR ALTER PROCEDURE aah_insert_missing_group_sponsors
AS
BEGIN;
SET NOCOUNT ON;
    ...
    SET COUNTY_ID = (SELECT TOP 1 COUNTY_ID FROM COUNTY WHERE NAME = 'Fort Bend')
    WHERE [NAME] = 'FOSTER AIR FORCE JUNIOR ROTC' AND COUNTY_ID IS NULL;

    IF ((SELECT COUNT(*) FROM @group_sponsor WHERE COUNTY_ID IS NULL) > 0)
    BEGIN;
