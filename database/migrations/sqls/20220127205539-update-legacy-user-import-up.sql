...
EXEC sp_executesql @stmt=N'IF ((SELECT OBJECT_ID(''GROUP_CONTACTS'',''U'')) IS NULL)
CREATE TABLE GROUP_CONTACTS (
	[GROUP_ID] INT NOT NULL FOREIGN KEY REFERENCES GROUP_SPONSOR([GROUP_ID]),
	[USER_ID] INT NOT NULL FOREIGN KEY REFERENCES USER_PERSON([USER_ID]),
	[IS_PRIMARY_CONTACT] BIT NOT NULL
);'

-- update the insert missing users
...
-- drop volunteers we ported over

-- First delete their role
DELETE r
FROM USER_PERSON u
LEFT JOIN GROUP_SPONSOR gs ON gs.GROUP_ID = CAST(LEFT(u.USERNAME, CHARINDEX(':', u.USERNAME) -1) AS INT)
LEFT JOIN ROLE r ON r.USER_ID = u.USER_ID
WHERE CHARINDEX(':', USERNAME) > 0 AND r.TYPE = 'Volunteer'
...
LEFT JOIN GROUP_SPONSOR gs ON gs.GROUP_ID = CAST(LEFT(u.USERNAME, CHARINDEX(':', u.USERNAME) -1) AS INT)
LEFT JOIN ROLE r ON r.USER_ID = u.USER_ID
WHERE CHARINDEX(':', USERNAME) > 0 AND r.ROLE_ID IS NULL
