/*
Update the join in the final update to correct for GUIDs
*/

CREATE OR ALTER
  PROCEDURE [aah].[aah_insert_missing_group_sponsors]
AS
BEGIN;
    SET NOCOUNT ON;
...
FROM gs_cursor INTO @grp_id;
    ...
    FROM @agreement t1
        ...
        INNER JOIN gis.[AAH_GIS_SEGMENTS] t2 ON CAST(t1.SEGMENT_ID as VARCHAR(255)) = CAST(t2.GlobalID AS VARCHAR(255))
